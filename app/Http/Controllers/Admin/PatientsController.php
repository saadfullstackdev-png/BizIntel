...
class PatientsController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.patients.index', compact('filters'));
    }
    ...
    public function datatable(Request $request)
    {
        ...
        if ($request->get('customActionType') && $request->get('customActionType') == "group_action") {
            $Patients = Patients::getBulkData($request->get('id'));
            $anyDeleted = false ;
            if ($Patients) {
        ...
        if ($Patients) {
            foreach ($Patients as $patient) {
                $records["data"][] = array(
                    'id' => '<label class="mt-checkbox mt-checkbox-single mt-checkbox-outline"><input name="id[]" type="checkbox" class="checkboxes" value="' . $patient->id . '"/><span></span></label>',
                    'name' => $patient->name,
                    'email' => $patient->email,
