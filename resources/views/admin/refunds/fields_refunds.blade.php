...
<div class="row">
    @if($is_wallet_checkbox_show == 1)
        <div class="form-group col-md-6">
            {!! Form::label('is_return_to_wallet', 'Return to wallet or not ?', ['class' => 'control-label']) !!}
            <br/>
            <label class="mt-checkbox">
                {!! Form::checkbox('is_return_to_wallet', 1, $is_checkbox_checked) !!} Is
                Return to Wallet ?
                <span></span>
            </label>
            @if($errors->has('is_return_to_wallet'))
                <p class="help-block">
                    {{ $errors->first('is_return_to_wallet') }}
                </p>
            @endif
        </div>
    @endif
