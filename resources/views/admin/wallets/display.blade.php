...
@section('title')
    <!-- BEGIN PAGE TITLE-->
    <h1 class="page-title">@lang('global.walletmetas.title')</h1>
    <!-- END PAGE TITLE-->
@endsection
        ...
        <div class="portlet-title">
            ...
            <div class="actions">
                @if(Gate::allows('wallets_addcash'))
                    <a class="btn btn-success btn-to-focus" href="{{ route('admin.wallets.add_cash', [$patient->id, $wallet->id]) }}" data-target="#ajax_wallets" data-toggle="modal">@lang('global.wallets.add_cash')</a>
                @endif
            </div>
        </div>
        <div class="portlet-body">
            <div class="table-container">
                <input type="hidden" id="wallet_id" value="{{ $id }}"/>
                <table class="table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
                    <thead>
                    <tr role="row" class="heading">
                        <th width="20%">@lang('global.walletmetas.fields.patient')</th>
                        <th>@lang('global.walletmetas.fields.phone')</th>
                        <th>@lang('global.walletmetas.fields.refund')</th>
                        <th>@lang('global.walletmetas.fields.cash_in')</th>
                        <th>@lang('global.walletmetas.fields.cash_out')</th>
{{--                        <th>@lang('global.walletmetas.fields.balance')</th>--}}
                        <th>@lang('global.walletmetas.fields.payment_mode')</th>
                        <th>@lang('global.walletmetas.fields.is_refund')</th>
                        <th>@lang('global.walletmetas.fields.is_reverse')</th>
                        <th>@lang('global.walletmetas.fields.created_at')</th>
                        <th>@lang('global.walletmetas.fields.actions')</th>
                    </tr>
...
{{--                        <td></td>--}}
                        ...
                        <td>
                            <div class="input-group date date-picker margin-bottom-5" data-date-format="yyyy-mm-dd">
                                <input type="text" class="form-control form-filter input-sm" readonly=""
                                       value="{{ ($filters->get(Auth::User()->id, 'walletmeta', 'created_from')) ? Carbon\Carbon::parse($filters->get(Auth::User()->id, 'walletmeta', 'created_from'))->format('Y-m-d') : '' }}"
                            ...
                            <div class="input-group date date-picker" data-date-format="yyyy-mm-dd">
                                <input type="text" class="form-control form-filter input-sm" readonly=""
                                       name="created_to"
                                       value="{{ ($filters->get(Auth::User()->id, 'walletmeta', 'created_to')) ? Carbon\Carbon::parse($filters->get(Auth::User()->id, 'walletmeta', 'created_to'))->format('Y-m-d') : '' }}"
                                       placeholder="To">
                                <span class="input-group-btn">
                                    <button class="btn btn-sm default" type="button">
                                        <i class="fa fa-calendar"></i>
    ...
    <div class="modal fade" id="ajax_wallets" role="basic" aria-hidden="true">
...
@section('javascript')
    ...
    <script src="{{ url('js/admin/walletmeta/datatable.js') }}" type="text/javascript"></script>
