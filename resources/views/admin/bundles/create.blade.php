...
<div class="modal-body">
    <div class="portlet-body form">
        <div class="form-group">
            {!! Form::open(['method' => 'POST', 'id' => 'form-validation', 'enctype' => 'multipart/form-data', 'route' => ['admin.bundles.store']]) !!}
            <div class="form-body">
            ...
            <!-- Ends Form Validation Messages -->
                ...
                <div class="row">
                    ...
                    <div class="form-group col-md-6">
                        ...
                        {!! Form::number('price', old('price'), ['id' => 'price', 'min' => '0', 'step' => '1', 'class' => 'form-control', 'placeholder' => '', 'required' => '']) !!}
                        @if($errors->has('price'))
                            <p class="help-block">
                                {{ $errors->first('price') }}
                            </p>
                ...
                <div class="row">
                    <div class="col-md-6 form-group">
                        {!! Form::label('Select Image', 'Package Image', ['class' => 'control-label']) !!}
                        <br>
                            ...
                            @foreach($tax_treatment_types as $tax_treatment_type)
                                <label class="mt-radio">{{$tax_treatment_type->name}}
                                    <input type="radio" value="{{$tax_treatment_type->id}}"
                                           name="tax_treatment_type_id" {{ ($tax_treatment_type->id == 1)? "checked" : "" }}/>
                                    <span></span>
                                </label>
                            @endforeach
                        </div>
                            ...
                            @foreach($content_display_types as $content_display_type)
                                <label class="mt-radio">{{$content_display_type->name}}
                                    <input type="radio" value="{{$content_display_type->id}}"
                                           name="is_mobile" {{ ($content_display_type->id == 1)? "checked" : "" }}/>
                                    <span></span>
                                </label>
                            @endforeach
                        </div>
...
<input type="hidden" value="0" id="total_servicesCount"/>
