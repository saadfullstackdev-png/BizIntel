...
<div class="row">
    ...
    <div class="form-group col-md-3">
        ...
        <select name="appointment_id" id="appointment_id" class="form-control select2 appointment_id">
    ...
    <div class="form-group col-md-2">
        ...
        {!! Form::select('lead_source_id',$lead_sources ,old('lead_source_id'),['class' => 'form-control select2 lead_source_id','id'=>'lead_source_id', "disabled" => true]) !!}
    </div>
    <div class="form-group col-md-2">
        <br><br>
...
<div class="row">
    <div class="form-group col-md-6">
        ...
        <select name="service_id" id="service_id" class="form-control select2 service_id">
...
<div class="row">
    <div class="form-group col-md-12">
        ...
        <select name="reference_id" id="reference_id" class="form-control select2 reference_id">
        ...
        @if($packagebundles)
            @foreach($packagebundles as $packagebundles)
                <tr class="HR_{{$packagebundles->id}}">
                    ...
                    <td><?php echo number_format($packagebundles->bundle->price) ?></td>
                    <td>
                        <?php if ($packagebundles->discount_id == null) {
                            echo '-';
                        } else {
                            echo $packagebundles->discount->name;
                        } ?>
                    </td>
                    ...
                    <td><?php echo number_format($packagebundles->tax_exclusive_net_amount); ?></td>
                    <td>{{$packagebundles->tax_percenatage}}</td>
                    <td>{{number_format($packagebundles->tax_including_price)}}</td>
                    <td>
                        <input type='hidden' class='package_bundles' name='package_bundles[]'
                               value='{{$packagebundles->id}}'/>
                        @if(Gate::allows('patients_plan_service_delete'))
                ...
                @foreach ($packageservices as $packageservice)
                    @if($packageservice->package_bundle_id == $packagebundles->id )
                        ...
                        <tr class="HR_{{$packagebundles->id}} {{$packagebundles->id}}" style="display: none;">
                            ...
                            <td>Amount : {{number_format($packageservice->tax_exclusive_price)}}</td>
                            <td>Tax % : {{$packageservice->tax_percenatage}}</td>
                            <td>Tax Amt. : {{number_format($packageservice->tax_including_price)}}</td>
...
<div class="row bottom-data">
    <div class="form-group col-md-3">
        ...
        <input type="text" name="package_total" id="package_total" value="{{number_format($package->total_price)}}"
                        ...
                        <td>
                            @if(!$packageadvances->wallet_id && $packageadvances->paymentmode->type != 'mobile')
                                @if($end_previous_date<=$packageadvances->created_at)
                                    @if(Gate::allows('patients_plan_cash_edit'))
                                        <a class="btn btn-xs btn-info"
                                           href="{{ route('admin.plans.edit_cash',[$packageadvances->id,$package->id]) }}"
                                           data-target="#plan_edit_cash"
                                           data-toggle="modal">@lang('global.app_edit')</a>
                                    @endif
                                    {{--$packageadvances->id,$package->id--}}
                                    @if(Gate::allows('patients_plan_cash_delete'))
