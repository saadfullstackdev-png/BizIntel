...
@section('title')
    <!-- BEGIN PAGE TITLE-->
    <h1 class="page-title">@lang('global.regions.title')</h1>
    <!-- END PAGE TITLE-->
@endsection
...
@section('content')
        ...
        <div class="portlet-title">
            ...
            <div class="actions">
                @if(Gate::allows('regions_create'))
                    <a class="btn btn-success btn-to-focus" href="{{ route('admin.regions.create') }}" data-target="#ajax_regions" data-toggle="modal">@lang('global.app_add_new')</a>
                @endif
                @if(Gate::allows('regions_sort'))
                    <a href="{{ route('admin.regions.sort') }}" class="btn btn-success">@lang('global.app_sort_region')</a>
                @endif
            </div>
        </div>
            ...
            <div class="table-container">
                ...
                <table class="table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
                    ...
                    <tr role="row" class="heading">
                        ...
                        <th>@lang('global.regions.fields.name')</th>
                        <th>@lang('global.regions.fields.status')</th>
                        <th width="20%">@lang('global.regions.fields.actions')</th>
                    </tr>
                    <tr role="row" class="filter">
                        <td></td>
                        <td>
                            <input type="text" class="form-control form-filter input-sm" name="name" value="{{ $filters->get(Auth::User()->id, 'regions', 'name') }}">
                        </td>
                        <td> {!! Form::select('status', ['0' => 'Inactive', '1' => 'Active'], $filters->get( Auth::user()->id, 'regions', 'status'), ['class' => 'form-control form-filter input-sm select2', 'placeholder' => 'All']) !!}</td>
                        <td>
    ...
    <div class="modal fade" id="ajax_regions" role="basic" aria-hidden="true">
...
@section('javascript')
    ...
    <script src="{{ url('js/admin/regions/datatable.js') }}" type="text/javascript"></script>
