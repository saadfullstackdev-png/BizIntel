...
<div class="modal-body">
    <div class="portlet-body form">
        <div class="form-group">
            {!! Form::model($staff_target, ['method' => 'PUT', 'id' => 'form-validation', 'route' => ['admin.staff_targets.update', $staff_target->id]]) !!}
                ...
                <div class="form-body">
                    ...
                    <div class="row">
                        <div class="col-md-6 form-group">
                            ...
                            <select id="year" name="year" class="form-control" onchange="CreateFormValidation.loadEndServices();" required>
                                @foreach($years as $key => $val)
                                    <option value="<?php echo $key ?>" @if($staff_target->year == $key) selected="selected" @endif><?php echo $val ?></option>
                                @endforeach
                            </select>
                            @if($errors->has('year'))
                                <p class="help-block">
                        ...
                        <div class="col-md-6 form-group">
                            ...
                            {!! Form::select('month', $months, $staff_target->month, ['onchange' => 'CreateFormValidation.loadEndServices();', 'class' => 'form-control', 'placeholder' => '', 'required' => '']) !!}
                            @if($errors->has('month'))
                                <p class="help-block">
                                    {{ $errors->first('month') }}
                                </p>
                            ...
                            <div class="input-group">
                                <select id="staff_id" name="staff_id" class="form-control">
                                    ...
                                    @foreach($staffs as $staff)
                                        <option value="<?php echo $staff->user_id ?>" @if($staff_target->staff_id == $staff->user_id) selected="selected" @endif><?php echo $staff->full_name ?></option>
                                    @endforeach
                                </select>
