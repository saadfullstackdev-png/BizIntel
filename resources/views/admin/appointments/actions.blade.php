<a class="btn btn-xs btn-warning" href="{{ route('admin.appointments.detail',[$appointment->app_id]) }}"
...
@if(Gate::allows('appointments_edit'))
    @if($appointment->appointment_type_id==1)
        <a class="btn btn-xs btn-info" href="{{ route('admin.appointments.edit',[$appointment->app_id]) }}"
    ...
    @elseif($appointment->appointment_type_id==2 && $is_editable)
        <a class="btn btn-xs btn-info" href="{{ route('admin.appointments.edit_service',[$appointment->app_id]) }}"
           data-target="#ajax_appointments_edit" data-toggle="modal" title="Edit"><i class="fa fa-edit"></i></a>
    @endif
@endif

<a href="{{ route('admin.appointments.sms_logs',[$appointment->app_id])  }}" class="btn btn-xs btn-success"
   data-target="#ajax_logs" data-toggle="modal"><i class="fa fa-send" data-toggle="tooltip" title="SMS Logs"></i></a>

<a href="{{ route('admin.appointments.notification_logs',[$appointment->app_id])  }}" class="btn btn-xs btn-success"
   data-target="#ajax_logs" data-toggle="modal"><i class="fa fa-bell" data-toggle="tooltip" title="Notification Logs"></i></a>
...
@if($cancelled_appointment_status && ($cancelled_appointment_status->id != $appointment->appointment_status_id))
    @if($appointment->appointment_type_id==1)
        @if(Gate::allows('appointments_consultancy'))
            <a href="{{route("admin.appointments.create")}}?id={{$appointment->app_id}}&city_id={{$appointment->city_id}}&location_id={{$appointment->location_id}}&doctor_id={{$appointment->doctor_id}}"
    ...
    @elseif($appointment->appointment_type_id==2)
        @if(Gate::allows('appointments_services'))
            <a href="{{route("admin.appointments.manage_services")}}?id={{$appointment->app_id}}&city_id={{$appointment->city_id}}&location_id={{$appointment->location_id}}&doctor_id={{$appointment->doctor_id}}&machine_id={{$appointment->resource_id}}"
               class="btn btn-xs btn-success"><i class="fa fa-calendar" data-toggle="tooltip"
                                                 title="Doctor Calendar of this appointment"></i></a>
        @endif
    @endif
...
@if(Gate::allows('appointments_invoice'))
    @if(!$invoice)
        @if($appointment->appointment_type_id == Config::get('constants.appointment_type_service'))
            <a class="btn btn-xs btn-info" href="{{ route('admin.appointments.invoicecreate',[$appointment->app_id]) }}"
        ...
        @if($appointment->appointment_type_id == Config::get('constants.appointment_type_consultancy'))
            <a class="btn btn-xs btn-info" href="{{ route('admin.appointments.invoice-create-consultancy',[$appointment->app_id]) }}"
...
@if($appointment->appointment_type_id==2)
    @if(Gate::allows('appointments_image_manage'))
        <a class="btn btn-xs btn-info"
           href="{{ route('admin.appointmentsimage.imageindex',[$appointment->app_id]) }}" target="_blank"><i
                    class="fa fa-file-image-o" title="Images"></i></a>
    @endif
    @if(Gate::allows('appointments_measurement_manage'))
        <a class="btn btn-xs btn-info"
           href="{{ route('admin.appointmentsmeasurement.measurements',[$appointment->app_id]) }}" target="_blank"><i
                    class="fa fa-stethoscope" title="Measurement"></i></a>
    @endif
@endif
@if($appointment->appointment_type_id==1)
    @if(Gate::allows('appointments_medical_form_manage'))
        <a class="btn btn-xs btn-info" href="{{ route('admin.appointmentsmedical.medicals',[$appointment->app_id]) }}"
           target="_blank"><i
                    class="fa fa-medkit" title="Medical History Form"></i></a>
    @endif
@endif
@if(Gate::allows('appointments_plans_create'))
    <a class="btn btn-xs btn-info" href="{{ route('admin.appointmentplans.create',[$appointment->app_id]) }}"><i class="fa fa-clipboard" title="Create Plan"></i></a>
@endif
@if(Gate::allows('appointments_destroy'))
    @if(
        ...
        {!! Form::open(array(
            ...
            'route' => ['admin.appointments.destroy', $appointment->app_id])) !!}
...
@if (Gate::allows('appointments_log'))
    <a class="btn btn-xs btn-info" target="_blank"
    href="{{ route('admin.appointments.viewlog', [$appointment->app_id, 'web']) }}"><i class="fa fa-history" title="{{ trans('global.app_log') }}"></i>
    </a>
@endif