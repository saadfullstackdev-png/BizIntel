...
@section('title')
    <!-- BEGIN PAGE TITLE-->
    <h1 class="page-title">@lang('global.machinetypes.title')</h1>
    <!-- END PAGE TITLE-->
@endsection
...
@section('content')
        ...
        <div class="portlet-title">
            ...
            <div class="actions">
                @if(Gate::allows('machineType_create'))
                    <a class="btn btn-success btn-to-focus" href="{{ route('admin.machinetypes.create') }}" data-target="#ajax_machinetypes" data-toggle="modal">@lang('global.app_add_new')</a>
                @endif
            </div>
        </div>
            ...
            <div class="table-container">
                ...
                <table class="table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
                    ...
                    <tr role="row" class="heading">
                        ...
                        <th>@lang('global.machinetypes.fields.name')</th>
                        <th>@lang('global.machinetypes.fields.services')</th>
                        <th>@lang('global.machinetypes.fields.created_at')</th>
                        <th>@lang('global.machinetypes.fields.status')</th>
                        <th>@lang('global.machinetypes.fields.actions')</th>
                    </tr>
                    <tr role="row" class="filter">
                        <td></td>
                        <td>
                            <input type="text" class="form-control form-filter input-sm" value="{{ $filters->get(Auth::User()->id, 'machinetypes', 'name') }}" name="name">
                        </td>
                        <td>
                            <select class="form-control form-filter input-sm select2" id="service" name="service">
                                ...
                                @foreach($Services as $id => $Service)
                                    ...
                                    <option @if ($filters->get(Auth::User()->id, 'machinetypes', 'service') == $id)  selected="selected" @endif value="@if($id < 0){{ ($id * -1) }}@else{{ $id }}@endif">{!! $Service['name'] !!}</option>
                                @endforeach
                            </select>
                        </td>
                        <td>
                            <div class="input-icon input-icon-sm right margin-bottom-5">
                                ...
                                <input name="created_from" data-date-format="yyyy-mm-dd" type="text" readonly=""
                                       class="form-control form-filter input-sm created_from" placeholder="From" value="{{ ($filters->get(Auth::User()->id, 'machinetypes', 'created_from')) ? Carbon\Carbon::parse($filters->get(Auth::User()->id, 'machinetypes', 'created_from'))->format('Y-m-d') : '' }}">
                            </div>
                            <div class="input-icon input-icon-sm right margin-bottom-5">
                                <i class="fa fa-calendar"></i>
                                <input name="created_to" data-date-format="yyyy-mm-dd" type="text" readonly=""
                                       class="form-control form-filter input-sm created_to" placeholder="To" value="{{ ($filters->get(Auth::User()->id, 'machinetypes', 'created_to')) ? Carbon\Carbon::parse($filters->get(Auth::User()->id, 'machinetypes', 'created_to'))->format('Y-m-d') : '' }}">
                            </div>
                        </td>
                        <td> {!! Form::select('status', ['0' => 'Inactive', '1' => 'Active'] , $filters->get( Auth::user()->id, 'machinetypes', 'status') , ['class' => 'form-control form-filter input-sm select2', 'placeholder' => 'All']) !!} </td>
                        <td>
    ...
    <div class="modal fade" id="ajax_machinetypes" role="basic" aria-hidden="true">
...
@section('javascript')
    ...
    <script src="{{ url('js/admin/machinetypes/datatable.js') }}" type="text/javascript"></script>
