...
@section('stylesheets')
    ...
    <style type="text/css">
        .termsandpolicy span.select2-container {
            z-index: 10050;
        }
    </style>
@stop
...
@section('title')
    <!-- BEGIN PAGE TITLE-->
    <h1 class="page-title">@lang('global.termsandpolicies.title')</h1>
    <!-- END PAGE TITLE-->
@endsection
...
@section('content')
        ...
        <div class="portlet-title">
            ...
            <div class="actions">
                @if(Gate::allows('termsandpolicies_create'))
                    <a class="btn btn-success btn-to-focus" href="{{ route('admin.termsandpolicies.create') }}" data-target="#ajax_termsandpolicies" data-toggle="modal">@lang('global.app_add_new')</a>
                @endif
                <!-- @if(Gate::allows('termsandpolicies_sort'))
                    <a href="{{ route('admin.termsandpolicies.sort') }}" class="btn btn-success">@lang('global.app_orderlist')</a>
                @endif -->
            </div>
        </div>
            ...
            <div class="table-container">
                ...
                <table class="table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
                    ...
                    <tr role="row" class="heading">
                        ...
                        <th>@lang('global.termsandpolicies.fields.name')</th>
                        <th>@lang('global.termsandpolicies.fields.description')</th>
                        <th>@lang('global.termsandpolicies.fields.status')</th>
                        <th width="20%">@lang('global.termsandpolicies.fields.actions')</th>
                    </tr>
                    <tr role="row" class="filter">
                        <td></td>
                        <td>
                            <input type="text" class="form-control form-filter input-sm" value="{{ $filters->get(Auth::User()->id, 'termsandpolicies', 'name') }}" name="name">
                        </td>
                        <td>
                            <input type="text" class="form-control form-filter input-sm" value="{{ $filters->get(Auth::User()->id, 'termsandpolicies', 'description') }}" name="description">
                        </td>
                        <td> {!! Form::select('status', ['0' => 'Inactive', '1' => 'Active'], $filters->get( Auth::user()->id , 'termsandpolicies', 'status'), ['class' => 'form-control form-filter input-sm select2', 'placeholder' => 'All']) !!}</td>
                        <td>
    ...
    <div class="modal fade" id="ajax_termsandpolicies" role="basic" aria-hidden="true">
...
<script src="https://cdn.ckeditor.com/ckeditor5/35.2.1/classic/ckeditor.js"></script>
    ...
    <script src="{{ url('js/admin/termsandpolicies/datatable.js') }}" type="text/javascript"></script>
