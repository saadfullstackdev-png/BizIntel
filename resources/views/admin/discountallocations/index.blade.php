...
@section('content')
        ...
        <div class="portlet-title">
            ...
            <div class="actions">
                @if(Gate::allows('discountallocations_create'))
                    <a class="btn btn-success btn-to-focus" href="{{ route('admin.discountallocations.create') }}"
                       data-target="#ajax_discountallocations" data-toggle="modal">@lang('global.app_add_new')</a>
                @endif
                @if(Gate::allows('discountallocations_import'))
                    <a href="{{ route('admin.discountallocations.import') }}"
                       class="btn btn-success pull-right margin-r-5">@lang('global.discountallocations.import')</a>
                @endif
            </div>
        </div>
            ...
            <div class="table-container">
                ...
                <table class="table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
                    ...
                    <tr role="row" class="heading">
                        ...
                        <th>@lang('global.discountallocations.fields.employee')</th>
                        <th>@lang('global.discountallocations.fields.discount')</th>
                        <th>@lang('global.discountallocations.fields.year')</th>
                        <th>@lang('global.discountallocations.fields.created_at')</th>
                        <th>@lang('global.discountallocations.fields.status')</th>
                        <th width="20%">@lang('global.discountallocations.fields.actions')</th>
                    </tr>
                    <tr role="row" class="filter">
                        <td></td>
                        <td>
                            {!! Form::select('user_id', $users, $filters->get(Auth::User()->id, 'discountallocations', 'user_id'), ['class' => 'form-control form-filter input-sm select2']) !!}
                        </td>
                        <td>
                            {!! Form::select('discount_id', $discounts, $filters->get(Auth::User()->id, 'discountallocations', 'discount_id'), ['class' => 'form-control form-filter input-sm select2']) !!}
                        </td>
                        <td>
                            <input type="text" class="form-control form-filter input-sm"
                                   value="{{ $filters->get(Auth::User()->id, 'discountallocations', 'year') }}"
                                   name="year">
                        </td>
                        <td>
                            <div class="input-icon input-icon-sm right margin-bottom-5">
                                ...
                                <input name="created_from" data-date-format="yyyy-mm-dd" type="text" readonly=""
                                       ...
                                       value="{{ ($filters->get(Auth::User()->id, 'discountallocations', 'created_from')) ? Carbon\Carbon::parse($filters->get(Auth::User()->id, 'discountallocations', 'created_from'))->format('Y-m-d') : '' }}">
                            </div>
                            <div class="input-icon input-icon-sm right margin-bottom-5">
                                <i class="fa fa-calendar"></i>
                                <input name="created_to" data-date-format="yyyy-mm-dd" type="text" readonly=""
                                       ...
                                       value="{{ ($filters->get(Auth::User()->id, 'discountallocations', 'created_to')) ? Carbon\Carbon::parse($filters->get(Auth::User()->id, 'discountallocations', 'created_to'))->format('Y-m-d') : '' }}">
                            </div>
                        </td>
                        <td> {!! Form::select('status', ['0' => 'Inactive', '1' => 'Active'], $filters->get( Auth::user()->id , 'discountallocations', 'status'), ['class' => 'form-control form-filter input-sm select2', 'placeholder' => 'All']) !!}</td>
                        <td>
    ...
    <div class="modal fade" id="ajax_discountallocations" role="basic" aria-hidden="true">
...
@section('javascript')
    ...
    <script src="{{ url('js/admin/discountallocations/datatable.js') }}" type="text/javascript"></script>
