...
@section('title')
    <!-- BEGIN PAGE TITLE-->
    <h1 class="page-title">@lang('global.cities.title')</h1>
    <!-- END PAGE TITLE-->
@endsection
        ...
        <div class="portlet-title">
            ...
            <div class="actions">
                @if(Gate::allows('cities_create'))
                    <a class="btn btn-success btn-to-focus" href="{{ route('admin.cities.create') }}" data-target="#ajax_cities" data-toggle="modal">@lang('global.app_add_new')</a>
                @endif
                @if(Gate::allows('cities_sort'))
                    <a href="{{ route('admin.cities.sort') }}" class="btn btn-success">@lang('global.app_orderlist')</a>
                @endif
            </div>
        </div>
            ...
            <div class="table-container">
                ...
                <table class="table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
                    ...
                    <tr role="row" class="heading">
                        ...
                        <th>@lang('global.cities.fields.featured')</th>
                        <th>@lang('global.cities.fields.region')</th>
                        <th>@lang('global.cities.fields.status')</th>
                        <th width="20%">@lang('global.cities.fields.actions')</th>
                    </tr>
                    <tr role="row" class="filter">
                        <td></td>
                        <td>
                            <input type="text" class="form-control form-filter input-sm" value="{{ $filters->get(Auth::User()->id, 'cities', 'name') }}" name="name">
                        </td>
                        <td>
                            {!! Form::select('is_featured', array('' => 'All', 1 => 'Yes', 0 => 'No'), $filters->get(Auth::User()->id, 'cities', 'is_featured'), ['class' => 'form-control form-filter input-sm select2',]) !!}
                        </td>
                        <td>
                            {!! Form::select('region_id', $regions, $filters->get(Auth::User()->id, 'cities', 'region_id'), ['class' => 'form-control form-filter input-sm select2']) !!}
                        </td>
                        <td> {!! Form::select('status', ['0' => 'Inactive', '1' => 'Active'], $filters->get( Auth::user()->id , 'cities', 'status'), ['class' => 'form-control form-filter input-sm select2', 'placeholder' => 'All']) !!}</td>
                        <td>
...
@section('javascript')
    ...
    <script src="{{ url('js/admin/cities/datatable.js') }}" type="text/javascript"></script>
