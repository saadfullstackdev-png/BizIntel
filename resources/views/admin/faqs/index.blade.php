...
@section('title')
    <!-- BEGIN PAGE TITLE-->
    <h1 class="page-title">@lang('global.faqs.title')</h1>
    <!-- END PAGE TITLE-->
@endsection
...
@section('content')
        ...
        <div class="portlet-title">
            ...
            <div class="actions">
                @if(Gate::allows('faqs_create'))
                    <a class="btn btn-success btn-to-focus" href="{{ route('admin.faqs.create') }}" data-target="#ajax_faqs" data-toggle="modal">@lang('global.app_add_new')</a>
                @endif
                <!-- @if(Gate::allows('faqs_sort'))
                    <a href="{{ route('admin.faqs.sort') }}" class="btn btn-success">@lang('global.app_orderlist')</a>
                @endif -->
            </div>
        </div>
            ...
            <div class="table-container">
                ...
                <table class="table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
                    ...
                    <tr role="row" class="heading">
                        ...
                        <th>@lang('global.faqs.fields.category')</th>
                        <th>@lang('global.faqs.fields.question')</th>
                        <th>@lang('global.faqs.fields.answer')</th>
                        <th>@lang('global.faqs.fields.status')</th>
                        <th width="20%">@lang('global.faqs.fields.actions')</th>
                    </tr>
                    <tr role="row" class="filter">
                        <td></td>
                        <td>
                            {!! Form::select('category_id', $categories, $filters->get(Auth::User()->id, 'faqs', 'category_id'), ['class' => 'form-control form-filter input-sm select2']) !!}
                        </td>
                        <td>
                            <input type="text" class="form-control form-filter input-sm" value="{{ $filters->get(Auth::User()->id, 'faqs', 'question') }}" name="question">
                        </td>
                        <td>
                            <input type="text" class="form-control form-filter input-sm" value="{{ $filters->get(Auth::User()->id, 'faqs', 'answer') }}" name="answer">
                        </td>
                        <td> {!! Form::select('status', ['0' => 'Inactive', '1' => 'Active'], $filters->get( Auth::user()->id , 'faqs', 'status'), ['class' => 'form-control form-filter input-sm select2', 'placeholder' => 'All']) !!}</td>
                        <td>
    ...
    <div class="modal fade" id="ajax_faqs" role="basic" aria-hidden="true">
...
@section('javascript')
    ...
    <script src="{{ url('js/admin/faqs/datatable.js') }}" type="text/javascript"></script>
