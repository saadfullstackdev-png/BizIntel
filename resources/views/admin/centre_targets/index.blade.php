...
@section('title')
    <!-- BEGIN PAGE TITLE-->
    <h1 class="page-title">@lang('global.centre_targets.title')</h1>
    <!-- END PAGE TITLE-->
@endsection
...
@section('content')
        ...
        <div class="portlet-title">
            ...
            <div class="actions">
                @if(Gate::allows('centre_targets_create'))
                    <a class="btn btn-success btn-to-focus" href="{{ route('admin.centre_targets.create') }}" data-target="#ajax_centretarget" data-toggle="modal">@lang('global.app_add_new')</a>
                @endif
            </div>
        </div>
            ...
            <div class="table-container">
                ...
                <table class="table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
                    ...
                    <tr role="row" class="heading">
                        ...
                        <th>@lang('global.centre_targets.fields.year')</th>
                        <th>@lang('global.centre_targets.fields.month')</th>
                        <th>@lang('global.centre_targets.fields.created_at')</th>
                        <th width="20%">@lang('global.cities.fields.actions')</th>
                    </tr>
                    <tr role="row" class="filter">
                        <td></td>
                        <td>
                            {!! Form::select('year', $years, $filters->get(Auth::User()->id, 'centertarget', 'year'), ['class' => 'form-control form-filter input-sm select2']) !!}
                        </td>
                        <td>
                            {!! Form::select('month', $months, $filters->get(Auth::User()->id, 'centertarget', 'month'), ['class' => 'form-control form-filter input-sm select2']) !!}
                        </td>
                        <td>
                            <div class="input-icon input-icon-sm right margin-bottom-5">
                                ...
                                <input name="created_from" data-date-format="yyyy-mm-dd" type="text" readonly=""
                                       value="{{ ($filters->get(Auth::User()->id, 'centertarget', 'created_from')) ? Carbon\Carbon::parse($filters->get(Auth::User()->id, 'centertarget', 'created_from'))->format('Y-m-d') : '' }}" class="form-control form-filter input-sm created_from" placeholder="From">
                            </div>
                            <div class="input-icon input-icon-sm right margin-bottom-5">
                                <i class="fa fa-calendar"></i>
                                <input name="created_to" data-date-format="yyyy-mm-dd" type="text" readonly=""
                                       value="{{ ($filters->get(Auth::User()->id, 'centertarget', 'created_to')) ? Carbon\Carbon::parse($filters->get(Auth::User()->id, 'centertarget', 'created_to'))->format('Y-m-d') : '' }}" class="form-control form-filter input-sm created_to" placeholder="To">
                            </div>
                        </td>
                        <td>
    ...
    <div class="modal fade" id="ajax_centretarget" role="basic" aria-hidden="true">
    ...
    <div class="modal fade" id="ajax_centre_display" role="basic" aria-hidden="true">
        <div class="modal-content">
            <div class="modal-body">
                <span> &nbsp;&nbsp;Loading... </span>
            </div>
...
@section('javascript')
    ...
    <script src="{{ url('js/admin/centretarget/datatable.js') }}" type="text/javascript"></script>
