...
@section('title')
    <!-- BEGIN PAGE TITLE-->
    <h1 class="page-title">@lang('global.notification_templates.title')</h1>
    <!-- END PAGE TITLE-->
@endsection
...
@section('content')
        ...
        <div class="portlet-title">
            ...
            <div class="actions">
                @if(Gate::allows('notification_templates_create'))
                    <a class="btn btn-success btn-to-focus" href="{{ route('admin.notification_templates.create') }}" data-target="#ajax_notification_templates" data-toggle="modal">@lang('global.app_add_new')</a>
                @endif
            </div>
        </div>
            ...
            <div class="table-container">
                ...
                <table class="table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
                    ...
                    <tr role="row" class="heading">
                        ...
                        <th>@lang('global.notification_templates.fields.name')</th>
                        <th>@lang('global.notification_templates.fields.content')</th>
                        <th>@lang('global.notification_templates.fields.slug')</th>
                        <th>@lang('global.notification_templates.fields.image_url')</th>
                        <th>@lang('global.notification_templates.fields.status')</th>
                        <th width="20%">@lang('global.notification_templates.fields.actions')</th>
                    </tr>
                    <tr role="row" class="filter">
                        <td></td>
                        <td> <input type="text" class="form-control form-filter input-sm" value="{{ $filters->get(Auth::User()->id, 'notification_templates', 'name') }}" name="name"></td>
                        <td> <input type="text" class="form-control form-filter input-sm" value="{{ $filters->get(Auth::User()->id, 'notification_templates', 'content') }}" name="content"></td>
                        <td> <input type="text" class="form-control form-filter input-sm" value="{{ $filters->get(Auth::User()->id, 'notification_templates', 'slug') }}" name="slug"></td>
                        <td></td>
                        <td> {!! Form::select('status', ['0' => 'Inactive', '1' => 'Active'], $filters->get( Auth::user()->id , 'notification_templates', 'status'), ['class' => 'form-control form-filter input-sm select2', 'placeholder' => 'All']) !!}</td>
                        <td>
    ...
    <div class="modal fade" id="ajax_notification_templates" role="basic" aria-hidden="true">
...
@section('javascript')
    ...
    <script src="{{ url('js/admin/notification_templates/datatable.js') }}" type="text/javascript"></script>
