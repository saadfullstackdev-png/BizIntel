$(document).ready(function () {
    /*we need to proceed function to get package for specific patient*/
    getpackages();
...
function getpackageinfo(){
    ...
    $.ajax({
        type: 'get',
        url: route('admin.finances.getpackagesinfo'),
        data: {
            'id': package_id,
            'cash_amount':cash_amount
        },
...
function getpackages(){
    console.log('I am from Package Advances');
    $('#wrongMessage').hide();
    var patient_id = $('#patient_id_1').val();
    var div = $(this).parents();
    var op = " ";
    $.ajax({
        type: 'get',
        url: route('admin.finances.getpackages'),
        data: {'id': patient_id},
        success: function (resposne) {
            if (resposne.status == '1') {
                ...
                $('#package_id_1').find('option').remove().end().val('Select Package').append(dropdowndata);
            } else {
                $('#wrongMessage').show();
            }
        },
